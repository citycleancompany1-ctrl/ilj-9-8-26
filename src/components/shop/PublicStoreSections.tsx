...
export const ServicesSectionRenderer: React.FC<{
  ...
  // Render only real merchant services from the store catalogue (type === 'SERVICE')
  ...
  // Only add verified services created by merchant in Services tab
  catalogServices.forEach((p) => {
    if (!seenTitles.has(p.name.trim().toLowerCase())) {
      seenTitles.add(p.name.trim().toLowerCase());
      list.push({
        product: p,
        id: p.id,
        title: p.name,
        ...
        price: p.price,
        formattedPrice: typeof p.price === 'number' ? formatINR(p.price) : String(p.price),
        duration: p.unit || undefined,
        imageUrl: p.imageUrl || 'https://images.unsplash.com/photo-1581578731548-c64695cc6952?w=600',
        category: p.category,
      });
    }
  });
  ...
  return (
    <section id="services" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-14 space-y-6">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 bg-white p-6 rounded-2xl border border-gray-200 shadow-xs">
        <div>
          <div className="inline-flex items-center gap-1.5 bg-blue-100 text-blue-800 text-[10px] font-black px-2.5 py-0.5 rounded-sm uppercase tracking-wider">
            <Briefcase className="w-3.5 h-3.5" /> Professional Services
          </div>
          <h2 className="text-2xl sm:text-3xl font-black uppercase tracking-tight text-slate-900 font-['Outfit',sans-serif] mt-1">
            {config?.title || 'Our Services'} ({list.length})
          </h2>
          <p className="text-xs text-gray-500 mt-1">
            {config?.subtitle || 'Book verified expert services directly with zero middleman fee.'}
          </p>
        </div>

        <div className="text-xs text-gray-500 flex items-center gap-2">
          <span className="flex items-center gap-1 font-semibold text-emerald-700 bg-emerald-50 px-2.5 py-1 rounded-sm">
            <ShieldCheck className="w-3.5 h-3.5 text-emerald-600" /> Direct Merchant Guarantee
          </span>
        </div>
      </div>

      {/* Services Grid - Exactly identical responsive layout and styling as Product Cards */}
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2.5 sm:gap-3.5 md:gap-4 lg:gap-5 w-full">
        {list.map((service, idx) => {
          ...
          const whatsappUrl = getWhatsAppDirectUrl(shop.whatsapp || shop.phone, whatsappMsg);

          return (
            <div
              key={service.id || idx}
              onClick={() => onSelectService && onSelectService(service.product)}
              className="bg-white rounded-xl sm:rounded-2xl border border-gray-200/90 hover:border-blue-500/60 overflow-hidden shadow-xs hover:shadow-md transition-all flex flex-col justify-between group relative w-full cursor-pointer"
            >
              {/* Service Image & Badges (Matching Product Card Style) */}
              <div className="relative aspect-square w-full bg-slate-100 overflow-hidden">
                ...
                <div className="absolute top-2 left-2 sm:top-2.5 sm:left-2.5 flex flex-col gap-1 pointer-events-none">
                  <span className="bg-blue-600 text-white text-[9px] sm:text-[10px] font-black px-1.5 sm:px-2 py-0.5 rounded shadow-xs uppercase tracking-wider">
                    SERVICE
                  </span>
                  {service.duration && (
                    <span className="bg-slate-900/80 backdrop-blur-xs text-white text-[8px] sm:text-[9px] font-bold px-1.5 sm:px-2 py-0.5 rounded shadow-xs uppercase tracking-wider">
                      {service.duration}
                    </span>
                  )}
                </div>
              ...
              {/* Service Details (Clean: Name + Price with Cart Button) */}
              <div className="p-2.5 sm:p-3.5 flex-1 flex flex-col justify-between gap-2 sm:gap-3">
                <h3 className="text-xs sm:text-sm font-bold text-slate-900 line-clamp-2 leading-snug group-hover:text-blue-600 transition-colors font-['Outfit',sans-serif]">
                  {service.title}
                </h3>
                ...
                <div className="flex items-center justify-between pt-2 border-t border-gray-100 gap-1.5">
                  {isPriceHidden ? (
                    ...
                    <div className="min-w-0">
                      <span className="text-sm sm:text-base md:text-lg font-black text-blue-700 font-['Outfit',sans-serif] truncate block">
                        {service.formattedPrice}
                      </span>
                    </div>
                  )}

                  {/* Cart / Book Action Button */}
                  <div className="shrink-0" onClick={(e) => e.stopPropagation()}>
                    {isPriceHidden ? (
                      <a
                        href={getWhatsAppDirectUrl(
                          shop.whatsapp || shop.phone,
                          `Namaste ${shop.businessName}! Mujhe aapki "${service.title}" service ke charges aur details jaanni hain.`
                        )}
                        ...
                        title="Ask Service Fee on WhatsApp"
                      >
                      ...
                      inCart ? (
                        <div className="flex items-center gap-1 bg-blue-50 border border-blue-200 rounded-lg p-0.5 sm:p-1">
                          <button
                            type="button"
                            onClick={(e) => {
                              e.stopPropagation();
                              onRemoveFromCart && onRemoveFromCart(service.id);
                            }}
                            className="w-6 h-6 sm:w-7 sm:h-7 rounded-md bg-white text-slate-800 font-bold flex items-center justify-center hover:bg-gray-100 shadow-xs cursor-pointer"
                            title="Decrease quantity"
                          >
                            <Minus className="w-3 h-3" />
                          </button>
                          <span className="font-bold text-xs px-1 text-blue-950 font-mono">
                            {inCart.quantity}
                          </span>
                          <button
                            type="button"
                            onClick={(e) => {
                              e.stopPropagation();
                              onAddToCart && onAddToCart(service.product);
                            }}
                            className="w-6 h-6 sm:w-7 sm:h-7 rounded-md bg-blue-600 text-white font-bold flex items-center justify-center hover:bg-blue-700 shadow-xs cursor-pointer"
                        ...
                        <button
                          ...
                          title="Add to cart"
                          className="w-8 h-8 rounded-lg sm:rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold flex items-center justify-center shadow-xs transition-all active:scale-95 cursor-pointer"
                        >
