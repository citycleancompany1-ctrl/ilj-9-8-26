...
export const StoreItemsCarouselSection: React.FC<StoreItemsCarouselSectionProps> = ({
  ...
  return (
    <section id={id} className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-12 space-y-4">
      {/* 1. Header Bar: Strict 70% Heading / 30% View All Button on both Mobile & Desktop */}
      <div className="flex items-center justify-between gap-3 bg-white p-3.5 sm:p-5 rounded-2xl border border-gray-200/90 shadow-2xs">
        {/* 70% Heading Area */}
        <div className="w-[70%] basis-[70%] min-w-0 pr-1 sm:pr-2">
          <div className="inline-flex items-center gap-1.5 bg-orange-50 border border-orange-200/80 text-orange-800 text-[10px] font-black px-2 py-0.5 rounded-md uppercase tracking-wider">
            {isService ? (
              <Briefcase className="w-3 h-3 text-orange-600 shrink-0" />
            ) : (
          ...
          <h2 className="text-sm sm:text-xl md:text-2xl font-black uppercase tracking-tight text-slate-900 font-['Outfit',sans-serif] mt-1 truncate">
            {title} <span className="text-gray-400 text-xs sm:text-sm md:text-base font-bold">({items.length})</span>
          </h2>
        ...
        {/* 30% View All Button Area */}
        <div className="w-[30%] basis-[30%] flex items-center justify-end gap-2 shrink-0">
          {extraHeaderAction}
          <button
            type="button"
            onClick={() => setIsViewAllExpanded((prev) => !prev)}
            className="w-full max-w-[130px] sm:max-w-[150px] px-2.5 sm:px-4 py-2 sm:py-2.5 rounded-xl font-black text-[11px] sm:text-xs uppercase tracking-wider bg-orange-600 hover:bg-orange-700 active:scale-95 text-white transition-all shadow-xs flex items-center justify-center gap-1 cursor-pointer text-center"
            title={isViewAllExpanded ? 'Show 2-Row Carousel' : 'View All Items in Grid'}
          >
            <span className="truncate">{isViewAllExpanded ? 'Carousel' : 'View All'}</span>
            <ArrowRight className="w-3 h-3 sm:w-3.5 sm:h-3.5 shrink-0" />
          </button>
        </div>
      </div>

      {/* Optional Toolbar (Search/Filter) */}
      {filterToolbar && <div>{filterToolbar}</div>}

      {/* 2. Content: 2-Row Horizontal Carousel OR Expanded Grid */}
      {isViewAllExpanded ? (
        ...
        <div className="space-y-4 animate-in fade-in duration-200">
          ...
          <div className="flex justify-center pt-2">
            <button
              type="button"
              onClick={() => setIsViewAllExpanded(false)}
              className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-800 text-xs font-bold uppercase tracking-wider rounded-xl cursor-pointer transition-colors"
            >
